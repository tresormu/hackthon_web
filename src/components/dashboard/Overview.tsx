...
export const Overview = ({ searchQuery }: OverviewProps) => {
  ...
  const { patients, chwTasks } = state;
  const trendData = buildTrendData(patients);

  const summary = {
    totalPatients: patients.length,
    highRisk: patients.filter(p => p.status === 'High Risk').length,
    missedVisits: patients.reduce((sum, p) => sum + (p.misses ?? 0), 0),
    chwTasks: chwTasks.filter(t => t.status === 'Pending').length,
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Total Patients" value={summary.totalPatients} icon={Users} trend="+12%" color="bg-brand-500" delay={0.1} />
        <StatCard title="Pending Appointments" value={summary.missedVisits} icon={Calendar} trend="+5%" color="bg-amber-500" delay={0.2} />
        <StatCard title="High Risk Cases" value={summary.highRisk} icon={AlertTriangle} trend="-2%" color="bg-rose-500" delay={0.3} />
        <StatCard title="CHW Tasks" value={summary.chwTasks} icon={ArrowUpRight} trend="+8%" color="bg-emerald-500" delay={0.4} />
      </div>
          ...
          {[
            ...
            <div key={item.title} className="p-4 rounded-2xl border border-slate-100 bg-white hover:shadow-md transition-shadow">
              ...
              <div className="flex gap-2 mt-4">
                <button className="px-3 py-2 rounded-lg text-xs font-bold bg-slate-50 text-slate-700 hover:bg-slate-100 transition-colors">Assign CHW</button>
                <button className="px-3 py-2 rounded-lg text-xs font-bold bg-brand-600 text-white hover:bg-brand-700 transition-colors">View List</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card">
          ...
          <div className="space-y-6">
            {[
              { label: 'On-time Visits', color: 'bg-brand-500', pct: '85%', w: 'w-[85%]' },
              { label: 'Missed Visits', color: 'bg-rose-500', pct: '12%', w: 'w-[12%]' },
              { label: 'Follow-ups Pending', color: 'bg-amber-500', pct: '3%', w: 'w-[3%]' },
            ].map(({ label, color, pct, w }) => (
              <div key={label}>
                ...
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                  <div className={`${color} h-full ${w} rounded-full`}></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="card lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-slate-400 hover:text-brand-600 transition-colors text-xs font-bold uppercase tracking-widest">View Patient</button>
          </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-slate-400 hover:text-brand-600 transition-colors text-xs font-bold uppercase tracking-widest">Audit</button>
          </div>
      ...
      <div className="card">
        ...
        <div className="divide-y divide-slate-50">
          {priorityPatients.length > 0 ? (
            ...
            <div className="p-8 text-center text-slate-500 italic">No patients registered yet</div>
          )}
        </div>
      </div>
    </div>
